  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch health info
        const healthRes = await fetch("http://localhost:5000/api/users/health");
        const healthData: THealthInfo = await healthRes.json();
        setHealthInfo(healthData);

        // Fetch last inserted user
        const lastUserRes = await fetch("http://localhost:5000/api/lastUser");
          ...
          <Table aria-label="Our all users">
            ...
            <TableBody>
                <TableRow key={lastUser?._id}>
                  ...
                  <p>{moment(new Date(`${lastUser?.birthOfDate}`)).format('DD MMMM YYYY')}</p>
                  </TableCell>
                  <TableCell className="font-semibold">{lastUser?.nationality}</TableCell>
